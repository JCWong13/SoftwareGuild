...
public class SightingDaoDbImpl implements SightingDao {
    ...
    private static final String SQL_GET_LAST_TEN_SIGHTINGS
            = "select Sight.* from Sighting sight "
            + "order by sight.SightingDateTime desc "
            + "limit 10";
    ...
    public void updateSighting(Sighting sighting) {
        jt.update(SQL_DELETE_SUPERPERSON_SIGHTING_BY_SIGHTINGID, sighting.getSightingID());
    ...
    public List<Sighting> getLastTenSightings() {
        List<Sighting> sightings = jt.query(SQL_GET_LAST_TEN_SIGHTINGS, new SightingMapper());
        return associateLocationWithSighting(sightings);
    }
